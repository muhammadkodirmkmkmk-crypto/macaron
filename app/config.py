...
# Фото читает младшая модель — так дешевле. Точность держим не моделью, а способом:
# отдельный короткий вопрос только про номер + увеличенный верх кадра + сверка
# двух чтений между собой. Если нужна максимальная точность: VISION_MODEL=claude-sonnet-4-5
